...
const Appointment = () => {
  ...
  const getAvailableSlots = async () =>{
     ...
     for(let i=0; i<7; i++){
      ...
      currentDate.setDate(today.getDate()+i) //0 + today = today
  ...
  const bookAppointment = async()=>{
    ...
    try {
      ...
      //api call to book the appointmnet
  ...
  return docInfo && (
    <div>
        ...
        <div className='flex flex-col sm:flex-row gap-4'>
          ...
          <div className='flex-1 border bprder-gray-400 rounded-lg p-8 py-7 bg-white mx-2 sm:mx-0 mt-[-80px] sm:mt-0'>
          ...
          <div className='flex gap-3 items-center w-full overflow-x-scroll mt-4'>
            {
              docSlots.length && docSlots.map((item, index)=>(
                <div onClick={()=>setSlotIndex(index)} className={`text-center py-6 min-w-16 rounded-full cursor-pointer ${slotIndex === index ? 'bg-indigo-500 text-white' : 'border border-gray'}`} key={index}>
